...
bool Packet::isBarometerErrorFlagSet()
{
	return isErrorFlagSet(ERROR_FLAG_OFFSET_BAROMETER);
}

bool Packet::isMagnetometerErrorFlagSet()
{
	return isErrorFlagSet(ERROR_FLAG_OFFSET_MAGNETOMETER);
}

bool Packet::isAccelerometerErrorFlagSet()
{
	return isErrorFlagSet(ERROR_FLAG_OFFSET_ACCELEROMETER);
}

bool Packet::isAnemometerErrorFlagSet()
{
	return isErrorFlagSet(ERROR_FLAG_OFFSET_ANEMOMETER);
}

bool Packet::isWindVaneErrorFlagSet()
{
	return isErrorFlagSet(ERROR_FLAG_OFFSET_WIND_VANE);
}

bool Packet::isBatteryErrorFlagSet()
{
	return isErrorFlagSet(ERROR_FLAG_OFFSET_BATTERY);
}
...
bool Packet::isErrorFlagSet(uint8_t index)
{
	if(index > 7) {
		return false;
	}
	return (packet.errors & (1<< index));
}
